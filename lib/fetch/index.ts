...
export namespace SmartFetch {
  export interface GlobalConfig extends Omit<RequestInit, "body" | "method"> {
    baseUrl?: string;
    shouldThrow?: (response: any) => boolean
  }

  export interface LocalConfig extends RequestInit {
  }
  ...
  export function initSmartFetch(config: GlobalConfig) {
    window.__SMART_FETCH_CONFIG__ = config
  }

  export function getSmartFetchConfig() {
